...
SELECT upper(first_name),                                      --SELECT가 맨 마지막에 수행하니까 위 아래 결과가 달라진다. 
      salary
FROM employees
...
--문제8.
--전체 부서를 출력하려고 합니다. 순서는 부서이름이 긴 순서대로 출력해 보세오SELECT department_name
...
SELECT first_name, salary,
    replace(phone_number, '.', '-') phone_number, -- . -> -로 치환  ====================암기
    hire_date
FROM employees
