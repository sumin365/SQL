...
SELECT first_name, 
    salary, 
    salary * 12 year-- 레코드 내 모든 salary 컬럼에 동일 산술연산을 수행
FROM employees;

SELECT job_id * 12 
FROM employees; -- Error: 산술 연산은 수치 자료형에서만. jobid는 varchar
...
SELECT first_name,
    salary + (salary * nvl(commission_pct, 0)) commission -- commission_pct null -> 0
FROM employees;
...
FROM employees
ORDER BY department_id ASC, -- ASC 생략 가능, 1차 정렬 기준/ 여러개 정렬시 ,로 나누기
    salary DESC;
...
SELECT first_name, last_name,
    CONCAT(first_name, CONCAT(' ', last_name)) name, --형식이 CONCAT(S1,S2)
...
-- 정제.  그냥 설정한것  아래있는것 연습해볼려고
SELECT '  Oracle  ', '*****Database*****'
FROM dual;

SELECT LTRIM('  Oracle  '), --  왼쪽에 있는 빈 공간을 지워줌
    RTRIM('  Oracle  '),    -- 오른쪽에 있는 공백을 지워줌
    TRIM('*' FROM '*****Database*****'), -- 문자열 내에서 특정 문자를 제거.(TRIM: 다듬다라는 뜻)
    SUBSTR('Oracle Database', 8, 8), -- 문자열에서 8번째 글자부터 8문자를 추출 (첫번쨰가 0이 아니라 1이다!)
...
SELECT ABS(-3.14),  -- 절대값
    ...
    MOD(7, 3),  -- 나눗셈의 나머지
    POWER(2, 4),    -- 제곱: 2의 4제곱
    ROUND(3.5),     -- 소숫점 첫째자리 반올림  ROUND(3.5, 아무것도 없으니 정수로 나오게)
...
SELECT sysdate FROM employees;  --  테이블 내의 레코드 갯수만큼 출력-보통 HIREDATE관한 정보 찾을떄 사용함

SELECT sysdate, -- 시스템 날짜
    ADD_MONTHS(sysdate, 2), -- 오늘부터 2개월 후
    MONTHS_BETWEEN(TO_DATE('1999-12-31', 'YYYY-MM-DD'), sysdate),   -- 개월 차 /문자형->날자형 으로 변환
...
SELECT first_name, hire_date,
    ROUND(MONTHS_BETWEEN(sysdate, hire_date), 1) as months --반올림ROUND까지 하자 
FROM employees;
...
SELECT 
    TO_NUMBER('-12345', '99999PR')    --음수로는 어떻게 바꾸지 ?????02PPT 53/58P  =====================
FROM dual;
-- TO_DATE: 날짜 형태를 지닌 문자열 -> date 
SELECT 
    '2021-03-16 15:07',
    TO_DATE('2021-03-16 15:07', 'YYYY-MM-DD HH24:MI') --날짜 연산가능해짐
FROM dual;
...
SELECT first_name, department_id, 
    ...
    END team
FROM employees
ORDER BY team;  --TEAM이 임시로 만든 칼럼이지만 정렬도 가능하다. 